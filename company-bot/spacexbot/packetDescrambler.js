...
function packetDescrambler(seq, fragmentData, n) {
    let maxSeq = Math.max(...seq);
    let arrData= [...new Array(maxSeq+1)].map(x => []);
    let output = "";

    for (let i=0;i<seq.length;i++){
        arrData[seq[i]].push(fragmentData[i]);
    }
    for (let i=0;i<arrData.length;i++){
        ...
        for (let j=0;j<arrData[i].length;j++){
            if (arrData[i][j] in countDict){
                countDict[arrData[i][j]]++;
            } else {
                countDict[arrData[i][j]]=1;
            }
            if (countDict[arrData[i][j]]>maxCount){
                maxCount = countDict[arrData[i][j]];
                maxData = arrData[i][j];
            }
        }
        if (maxCount>Math.floor(n/2)){
            output+=maxData;
            if (maxData==="#"){//terminate
                if (i<maxSeq)//early terminate
                    return "";
                else
                    break;
            }
